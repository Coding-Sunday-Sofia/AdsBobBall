...
import eu.veldsoft.adsbobball.GameLogic.Bar;
import eu.veldsoft.adsbobball.GameLogic.BarSection;
...
public class GameView {
	...
	public void reset(GameState gameState) {
		...
		if (gameState.getGrid() != null) {
			...
			return;
		this.gridSquareSize = (int) Math.floor(Math.min(canvasWidth / maxX,
				canvasHeight / maxY));
	...
	public void draw(final Canvas canvas, GameState gameState) {
		...
		for (Player player : gameState.getPlayers()) {
			...
			List<RectF> collisionRects = gameState.getGrid().getCollisionRects(
					playerId);
			for (RectF rect : collisionRects) {
				canvas.drawRect(xOffset + rect.left * gridSquareSize, yOffset
						+ rect.top * gridSquareSize, xOffset + rect.right
						* gridSquareSize, yOffset + rect.bottom
						* gridSquareSize, paint);
			}
			...
			if (sectionOne != null) {
				...
				canvas.drawRect(xOffset + sectionOneRect.left * gridSquareSize,
						yOffset + sectionOneRect.top * gridSquareSize, xOffset
								+ sectionOneRect.right * gridSquareSize,
			...
			if (sectionTwo != null) {
				...
				canvas.drawRect(xOffset + sectionTwoRect.left * gridSquareSize,
						yOffset + sectionTwoRect.top * gridSquareSize, xOffset
								+ sectionTwoRect.right * gridSquareSize,
		...
		for (int i = 0; i < balls.size(); ++i) {
			...
			canvas.drawBitmap(circleBitmap, xOffset + ball.getX1()
					* gridSquareSize, yOffset + ball.getY1() * gridSquareSize,
					null);
		}
	}

	private void preCacheBackground(final Canvas canvas,
			final GameState gameState) {

		backgroundBitmap = Bitmap.createBitmap(canvasWidth, canvasHeight,
		...
		for (int x = 0; x < maxX; ++x) {
			for (int y = 0; y < maxY; ++y) {
				if (grid[x][y] == Grid.GRID_SQUARE_CLEAR) {
					bitmapCanvas.drawRect(xOffset + (x * gridSquareSize),
							yOffset + (y * gridSquareSize), xOffset
									+ ((x + 1) * gridSquareSize), yOffset
									+ ((y + 1) * gridSquareSize),
							Paints.gridPaint);
					bitmapCanvas.drawRect(xOffset + (x * gridSquareSize),
							yOffset + (y * gridSquareSize), xOffset
		...
		circleBitmap = Bitmap.createBitmap((int) gridSquareSize,
				(int) gridSquareSize, Bitmap.Config.ARGB_8888);
		Canvas circleBitmapCanvas = new Canvas(circleBitmap);
		float radius = gridSquareSize / 2.0f;
		circleBitmapCanvas.drawCircle(radius, radius, radius,
				Paints.circlePaint);
	}

	public PointF transformPix2Coords(PointF pix) {
		return new PointF((pix.x - xOffset) / gridSquareSize, (pix.y - yOffset)
				/ gridSquareSize);
	}
}
