...
public class GameEventNewGame extends GameEvent {
	...
	public GameEventNewGame(int time, int level, int seed, int rows, int cols,
			float ballspeed, float barspeed) {
		super(time);
		this.level = level;
		this.rows = rows;
		this.cols = cols;
	...
	private void makeBalls(GameState gs, final int numberOfBalls) {
		...
		do {
			collision = false;
			float xPoint = randomGenerator.nextFloat()
					* (grid.getWidth() * 0.5f) + (grid.getWidth() * 0.25f);
			float yPoint = randomGenerator.nextFloat()
					* (grid.getHeight() * 0.5f) + (grid.getHeight() * 0.25f);
			...
			Ball ball = new Ball(xPoint, yPoint, (float) verticalSpeed,
