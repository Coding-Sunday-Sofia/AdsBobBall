...
import eu.veldsoft.adsbobball.Utilities;

//data is recreated, so no parcelable implementation needed
class GridPerPlayer {
	public List<RectF> collisionRects = new ArrayList<RectF>();
...
public class Grid implements Parcelable {
	...
	public Grid(final int numberOfRows, final int numberOfColumns,
		...
		this.perPlayer = new ArrayList<GridPerPlayer>();
	...
	public Grid(Grid other) {
		...
		for (int i = 0; i < maxPlayerId; i++) {
			...
			pnew.collisionRects = new ArrayList<RectF>();
	...
	public int getPercentComplete(int playerId) {
		return ((perPlayer.get(playerId).filledGridSquares) * 100)
				/ totalGridSquares;
	}
		...
		for (int x = x1; x < x2; ++x) {
			for (int y = y1; y < y2; ++y) {
				if (x >= 0 && x < maxX && y >= 0 && y < maxY
						&& gridSquares[x][y] == GRID_SQUARE_CLEAR) {
			...
			for (int x = 0; x < maxX; ++x) {
				for (int y = 0; y < maxY; ++y) {
					if (tempGridSquares[x][y] == GRID_SQUARE_SAFELY_CLEAR) {
						...
						// to the left
						if (x > 0
								&& tempGridSquares[x - 1][y] == GRID_SQUARE_CLEAR) {
						...
						// upwards
						if (y > 0
								&& tempGridSquares[x][y - 1] == GRID_SQUARE_CLEAR) {
	...
	public void findLargestContiguousFilledArea(int x, int y, int playerid) {
		...
		for (int currentX = x; currentX < maxX && lineMatch; ++currentX) {
			for (int currentY = currentMinY; currentY <= currentMaxY
					&& lineMatch; ++currentY) {
				if (!((tempGridSquares[currentX][currentY] == GRID_SQUARE_FILLED
						+ playerid) || (tempGridSquares[currentX][currentY] == GRID_SQUARE_COMPRESSED
						+ playerid))) {
					lineMatch = false;
				}
			}
		...
		perPlayer.get(playerid).collisionRects.add(new RectF(currentMinX,
				currentMinY, (currentMaxX + 1), (currentMaxY + 1)));
	}
