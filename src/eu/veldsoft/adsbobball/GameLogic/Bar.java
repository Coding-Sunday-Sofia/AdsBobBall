...
public class Bar implements Parcelable {
	...
	public void start(final BarDirection barDirectionIn,
			final RectF gridSquareFrame) {

		if (active) {
			throw new IllegalStateException(
					"Cannot start an already started bar!");
		}
		...
		if (barDirection == BarDirection.VERTICAL) {
			sectionOne = new BarSection(gridSquareFrame.left,
					gridSquareFrame.top, gridSquareFrame.right,
					gridSquareFrame.top, BarSection.MOVE_UP, speed);
			sectionTwo = new BarSection(gridSquareFrame.left,
					gridSquareFrame.top, gridSquareFrame.right,
					gridSquareFrame.bottom, BarSection.MOVE_DOWN, speed);
		} else {
			sectionOne = new BarSection(gridSquareFrame.left,
					gridSquareFrame.top, gridSquareFrame.left,
					gridSquareFrame.bottom, BarSection.MOVE_LEFT, speed);
			sectionTwo = new BarSection(gridSquareFrame.left,
					gridSquareFrame.top, gridSquareFrame.right,
					gridSquareFrame.bottom, BarSection.MOVE_RIGHT, speed);
		}
	}
	...
	public static final Parcelable.Creator<Bar> CREATOR = new Parcelable.Creator<Bar>() {
		public Bar createFromParcel(Parcel in) {
			...
			bar.barDirection = (bd == 0) ? BarDirection.VERTICAL
