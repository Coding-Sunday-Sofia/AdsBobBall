...
public class Ball implements Parcelable {
	...
	public Ball(final float x, final float y, final float horizontalVelocity,
			final float verticalVelocity, final float size) {
		this.size = size;
	...
	public void collision(Ball other) {
		...
		if ((horizontalVelocity > 0 && xDistance > 0)
				|| (horizontalVelocity < 0 && xDistance < 0)) {
			horizontalVelocity = -horizontalVelocity;
		}

		if ((verticalVelocity > 0 && yDistance > 0)
				|| (verticalVelocity < 0 && yDistance < 0)) {
		...
		do {
			...
			pointOne.set(frame.left + radius, frame.top + radius);
			distanceSquared = ((pointOne.x - pointTwo.x) * (pointOne.x - pointTwo.x))
	...
	public boolean collide(Ball other) {
		...
		float distance = ((pointOne.x - pointTwo.x) * (pointOne.x - pointTwo.x))
				+ ((pointOne.y - pointTwo.y) * (pointOne.y - pointTwo.y));
	...
	public static final Parcelable.Creator<Ball> CREATOR = new Parcelable.Creator<Ball>() {
		public Ball createFromParcel(Parcel in) {
			...
			return new Ball(left, top, horizontalVelocity, verticalVelocity,
					size);
		}
