...
import eu.veldsoft.adsbobball.Player;
import eu.veldsoft.adsbobball.Actors.Actor;
import eu.veldsoft.adsbobball.Actors.StupidAIActor;
...
public class GameManager implements Parcelable, Runnable {
	...
	public GameManager() {
		...
		gameStates = new LinkedBlockingDeque<GameState>();
	...
	public synchronized void reset() {
		setGameTime(0);
		GameEvent ev = new GameEventNewGame(getGameTime(),
				getCurrGameState().level, seed, NUMBER_OF_ROWS,
				NUMBER_OF_COLUMNS, BAR_SPEED, INITIAL_BALL_SPEED);
			...
			if (player.level < gs.level) // update score
				if (player.getLives() > 0)
					player.setScore(player.getScore()
							+ (int) ((gs.getGrid().getPercentComplete(
									player.getPlayerId()) * (GameManager
									.timeLeft(gs) / 1000.0)) * gs.level));
		}
	...
	private static void moveBars(GameState gameState) {
		...
		for (int playerid = 0; playerid < players.size(); playerid++) {
			...
			for (int otherplayerid = 0; otherplayerid < players.size(); otherplayerid++) {
				List<RectF> collisionRectsList = grid
						.getCollisionRects(otherplayerid);
				List<RectF> sectionCollisionRects = bar
						.collide(collisionRectsList);
	...
	public synchronized void addEvent(GameEvent ev) {
		pendingGameEv.addEvent(ev);
		Log.d(TAG, "@" + getGameTime() + " added event, pending: "
				+ pendingGameEv);
	}

	private static GameState revertGameStateTo(int time,
			Deque<GameState> gameStates) {
	...
	// add a checkpoint and delete the oldest one, if the queues capacity is
	// reached
	...
	public synchronized void startGameLoop() {
		...
		threadpool.scheduleAtFixedRate(this, 0,
				(long) (1000.0f / NUMBER_OF_UPDATES_PER_SECOND),
				TimeUnit.MILLISECONDS);
		for (Actor a : actors) {
			float rate = a.getExecFreq();
			if (rate > 0)
				threadpool.scheduleAtFixedRate(a, 0,
						(long) (1000.0f / NUMBER_OF_UPDATES_PER_SECOND / rate),
						TimeUnit.MILLISECONDS);
			else
	...
	public synchronized void singleStepGameLoop() {
		...
		if (firstEvTime < getGameTime()) {
			...
			Log.d(TAG, "Rollback from " + getGameTime() + " to " + firstEvTime
					+ " pending:" + pendingGameEv);
		}
		...
		/*
		 * if (gs.time % 200 == 0) { Log.d(TAG, "pending now=" + gameTime + "  "
		 * + pendingGameEv); Log.d(TAG, "processed " + processedGameEv); }
		 */
	...
	private static void advanceGameState(final GameState gameState,
			final GameEventQueue pending, final GameEventQueue processed) {
		// apply all pending events at gameState.time and move the to the
		// processed list
		while (true) {
		...
		for (Ball ball : balls) {
			...
			for (Player player : gameState.getPlayers()) {
				if (player.bar.collide(ball)) {
					// Log.d(TAG, "@" + gameState.time +
					// " bar finished (wall) for player " + player.getPlayerId()
					// + " bar active:" + player.bar.isActive() +
					// player.bar.getSectionOne() + player.bar.getSectionTwo());
