...
import eu.veldsoft.adsbobball.Network.NetworkIP;

enum ActivityStateEnum {
	GAMEINTRO, GAMERUNNING, GAMEPAUSED, GAMELOST, GAMEWON
}

public class BobBallActivity extends Activity implements
		...
		statusBotright.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (secretHandshake > 4) {
					...
					final NetworkIP n = new NetworkIP((int) System
							.currentTimeMillis());
					n.startServer();
	...
	public SpannableStringBuilder formatPerPlayer(String fixed, playstat query) {
		...
		for (Player p : gameManager.getCurrGameState().getPlayers()) {
			if (p.getPlayerId() == 0)
				continue;
			SpannableString s = new SpannableString(String.valueOf(query
					.call(p)) + " ");
			s.setSpan(new ForegroundColorSpan(p.getColor()), 0, s.length(),
	...
	protected void update(final Canvas canvas, final GameState currGameState,
		...
		if (frameCounter % displayLoop.ITERATIONS_PER_STATUSUPDATE == 0) {
			...
			SpannableStringBuilder timeLeftStr = SpannableStringBuilder
					.valueOf(getString(R.string.timeLeftLabel,
							gameManager.timeLeft() / 10));

			SpannableStringBuilder livesStr = formatPerPlayer(
					getString(R.string.livesLabel), new playstat() {
						@Override
						public int call(Player p) {
							return p.getLives();
						}
					});
			SpannableStringBuilder scoreStr = formatPerPlayer(
					getString(R.string.scoreLabel), new playstat() {
						@Override
						public int call(Player p) {
							return p.getScore();
						}
			...
			SpannableStringBuilder clearedStr = formatPerPlayer(
					getString(R.string.areaClearedLabel), new playstat() {
						@Override
						public int call(Player p) {
							Grid grid = currGameState.getGrid();
							if (grid != null)
								return currGameState.getGrid()
										.getPercentComplete(p.getPlayerId());
							else
								return 0;
						}
					});
			...
			if (secretHandshake >= 3) {
				...
				int color = (fps < NUMBER_OF_FRAMES_PER_SECOND * 0.98f ? Color.RED
						: Color.GREEN);
				SpannableString s = new SpannableString(String.format(
						" FPS: %2.1f", fps));
				...
				color = (gameManager.getUPS() < gameManager.NUMBER_OF_UPDATES_PER_SECOND * 0.98f ? Color.RED
						: Color.GREEN);
				s = new SpannableString(String.format(" UPS: %3.1f",
						gameManager.getUPS()));
	...
	private void promptUsername() {
		...
		new AlertDialog.Builder(this).setTitle(R.string.namePrompt)
				.setMessage(R.string.highScoreAchieved).setView(input)
				.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int whichButton) {
						...
						scores.addScore(valueString, gameManager
								.getCurrGameState().getPlayer(playerId)
								.getScore());
						showTopScores();
					}
				}).show();
	...
	private void showWonScreen() {
		messageView.setText(getString(R.string.levelCompleted,
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		PointF evPoint = gameView.transformPix2Coords(new PointF(event.getX(),
				event.getY()));
	...
	public void onRestoreInstanceState(Bundle savedInstanceState) {
		...
		activityState = ActivityStateEnum.values()[savedInstanceState.getInt(
				STATE_ACTIVITY, 0)];
		reinitGame();
	}
	...
	private class DisplayLoop implements Runnable {
		...
		public void run() {
			...
			final GameState currGameState = new GameState(
					gameManager.getCurrGameState()); // make a copy of the state
														// so its immutable
			final Player currPlayer = currGameState.getPlayer(playerId);
			...
			if (gameManager.getGameTime() > lastGameTime) {
				...
				if (frameCounter % ITERATIONS_PER_STATUSUPDATE == 0) {
					...
					fps = (float) ITERATIONS_PER_STATUSUPDATE
							/ (currTime - displayLoop.fpsStatsLastTS) * 1e9f;
